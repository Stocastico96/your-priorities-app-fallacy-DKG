...
router.post("/:pointId/feedback", auth.isLoggedIn, async function (req, res) {
    try {
        ...
        const [feedback, created] = await models.sequelize.query(`INSERT INTO fallacy_feedback (point_id, fallacy_label, user_id, is_helpful, created_at, updated_at)
...
router.get("/:pointId/feedback", async function (req, res) {
    try {
        const { pointId } = req.params;
        const feedbackStats = await models.sequelize.query(`SELECT
          fallacy_label,
        ...
        let userFeedback = {};
        if (req.user) {
            const userFeedbackRows = await models.sequelize.query(`SELECT fallacy_label, is_helpful
           FROM fallacy_feedback
...
router.post("/:pointId/comments", auth.isLoggedIn, async function (req, res) {
    try {
        const { pointId } = req.params;
        ...
        const [result] = await models.sequelize.query(`INSERT INTO fallacy_comments (point_id, fallacy_label, user_id, comment, created_at, updated_at)
         VALUES ($1, $2, $3, $4, NOW(), NOW())
         RETURNING *`, {
...
router.get("/:pointId/comments", async function (req, res) {
    try {
        const { pointId } = req.params;
...
router.get("/:pointId/comment-counts", async function (req, res) {
    try {
        const { pointId } = req.params;
        const counts = await models.sequelize.query(`SELECT fallacy_label, COUNT(*) as count
