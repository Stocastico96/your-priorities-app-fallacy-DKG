...
const DELIB_STORAGE_DIR = path.resolve(process.env.DELIB_STORAGE_DIR || "storage/delib-annotations");
...
const perspectiveClient = process.env.GOOGLE_PERSPECTIVE_API_KEY
    ...
    : null;
const OPENAI_MODEL = process.env.OPENAI_STREAMING_MODEL_NAME || "deepseek/deepseek-chat-v3.1:free";
...
const FALLACY_BLOCK_THRESHOLD = parseFloat(process.env.PERSPECTIVE_BLOCK_THRESHOLD || "0.85");
const FALLACY_SOFT_THRESHOLD = parseFloat(process.env.PERSPECTIVE_SOFT_THRESHOLD || "0.65");
...
const persistPerspective = async (context, moderation) => {
    if (!moderation)
        return;
    const targetId = context.contentType === "comment"
        ? context.commentId
    ...
    await models.ModerationPerspective.create({
        ...
        identity_attack: moderation.scores?.IDENTITY_ATTACK?.summaryScore?.value,
        sexually_explicit: moderation.scores?.SEXUALLY_EXPLICIT?.summaryScore?.value,
...
const persistDelibResult = async (context, analysis) => {
    ...
    if (delibResult.ontologyHints && delibResult.ontologyHints.jsonld) {
        ...
        const filePath = path.join(DELIB_STORAGE_DIR, `${context.contentType}-${targetId}-${uuidv4()}.jsonld`);
        await fs.writeFile(filePath, JSON.stringify(delibResult.ontologyHints.jsonld, null, 2), "utf-8");
        ...
        await queue.add("delibai-analysis", {
            analysisId: savedAnnotation.id,
            jsonld: delibResult.ontologyHints.jsonld,
        }, "low");
        await queue.add("delib-ai-ontology-index", {
            context,
            jsonld: delibResult.ontologyHints.jsonld,
            filePath,
        }, "low");
    }
    await queue.add("delib-ai-fallacy-log", {
        context,
        delibResult,
    }, "low");
};
