...
router.delete(
  "/:postId/:activityId/delete_activity",
  auth.can("edit post"),
  function (req, res) {
...
router.post(
  "/:id/status_change_no_emails",
  auth.can("send status change"),
  function (req, res) {
    models.Post.findOne({
      where: {
        id: req.params.id,
    ...
    models.Post.findOne({
      ...
      if (post) {
        models.PostStatusChange.build({
          post_id: post.id,
          status_changed_to:
            post.official_status != parseInt(req.body.official_status)
...
router.get("/:id/translatedSurvey", auth.can("view post"), function (req, res) {
  ...
  async.parallel(
    [
      ...
      (parallelCallback) => {
        models.AcTranslationCache.getSurveyAnswerTranslations(
          req.params.id,
          targetLanguage,
          (error, translations) => {
            if (error) {
...
router.get(
  "/:id/:statusId/translatedStatusText",
  auth.can("view post"),
  function (req, res) {
        ...
        models.Point.findAndCountAll({
          ...
          order: [
            models.sequelize.literal(
              "(counter_quality_up-counter_quality_down) desc"
            ),
          ],
          limit: 100,
...
var updatePostData = function (req, post) {
  ...
  if (req.body.publicPrivateData) {
    try {
      post.set(
        ...
        JSON.parse(req.body.publicPrivateData)
      );
    } catch (error) {
  ...
  if (req.body.uploadedDocumentUrl && req.body.uploadedDocumentUrl != "") {
    post.set(
      "data.attachment.url",
      req.body.uploadedDocumentUrl && req.body.uploadedDocumentUrl != ""
  ...
  if (req.body.structuredAnswersJson && req.body.structuredAnswersJson != "") {
    try {
      post.set(
        ...
        JSON.parse(req.body.structuredAnswersJson)
      );
    } catch (e) {
      ...
      post
        ...
        .then(function () {
          ...
          queue.add(
            ...
            { type: "update-collection", postId: post.id },
            "low"
          );
    ...
    models.Post.findOne({
      ...
      .then((post) => {
        if (post && post.public_data && post.public_data.transcript) {
          if (
            post.public_data.transcript.inProgress === true &&
            ...
            if (post.public_data.transcript.inProgressDate) {
              startDate = moment(
                post.public_data.transcript.inProgressDate
              ).toDate();
            }
            if (
              !post.public_data.transcript.inProgressDate ||
              ...
              models.Video.findOne({
                ...
                .then((video) => {
                  if (video.meta.transcript && video.meta.transcript.text) {
                    post.set("public_data.transcript.inProgress", false);
                    post.set(
                      "public_data.transcript.text",
                      video.meta.transcript.text
                    );
                    post
                      .save()
                      .then((savedPost) => {
                        log.info(
                          "process-moderation post toxicity after video transcript"
                        );
                        queue.add(
                          "process-moderation",
                          {
              ...
              models.Audio.findOne({
                ...
                .then((audio) => {
                  if (audio.meta.transcript && audio.meta.transcript.text) {
                    ...
                    post.set(
                      "public_data.transcript.text",
                      audio.meta.transcript.text
                    );
                    post
                      .save()
                      .then((savedPost) => {
                        log.info(
                          "process-moderation post toxicity after audio transcript"
                        );
                        queue.add(
                          "process-moderation",
                          {
    ...
    .then(function (post) {
      if (post) {
        ...
        post.save().then(function () {
          ...
          queue.add(
            "process-moderation",
            { type: "estimate-post-toxicity", postId: post.id },
            "high"
          );
          queue.add(
            "process-moderation",
            { type: "post-review-and-annotate-images", postId: post.id },
            "medium"
          );
        ...
        post.updateAllExternalCounters(
          ...
          function () {
            ...
            models.Point.count({
              ...
              post.updateAllExternalCountersBy(
                req,
                "down",
                "counter_points",
                countInfo,
...
router.put(
  "/:postId/:campaignId/update_campaign",
  auth.can("edit post"),
  async (req, res) => {
    try {
...
router.delete(
  "/:postId/:campaignId/delete_campaign",
  auth.can("edit post"),
  async (req, res) => {
    try {
      const campaign = await models.Campaign.findOne({
